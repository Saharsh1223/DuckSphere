...
class Puzzle(commands.Cog):
    ...
    async def puzzle(self, interaction: discord.Interaction, rating_deviation: int = 100):
        ...
        if (
            (puzzle_id is not None
            or puzzle_id != "")
            and puzzle_id != puzzle_data['Puzzle ID']
        ):
            await interaction.followup.send('Error: You are already in a puzzle!')
            return
        ...
        if len(moves) > 1:
            ...
            moves_to_solve = [element for index, element in enumerate(moves) if index % 2 == 0]
        else:
            opponent_moves = []
            moves_to_solve = []
        ...
        puzzle_id = puzzle_data['Puzzle ID']  # Store puzzle_id directly

        if not db.collection('puzzles').document(puzzle_id).get().exists:
            db.collection('puzzles').document(puzzle_id).set(
                {
                    'puzzle_id': puzzle_id,
                    'moves': moves,
                    'opponent_moves': opponent_moves,
                    'moves_to_solve': moves_to_solve,
                    'start_fen': f"{b.board_fen()} {t}",
                    'end_fen': f"{puzzle_data['End_FEN']} {t}",
                }
            )
        ...
        userdoc_ref.update(
            {
                'puzzle_id': puzzle_id,
                'puzzle_fen': f"{b.board_fen()} {t}",
                'puzzle_turn': t,
                'stage': 0
            }
        )

        # db.collection('puzzles').document(puzzle_id).collection('members').document(str(interaction.user.id)).set(
        #     {
        #         'stage': 0,
        ...
        await interaction.followup.send(f"Solution: {db.collection('puzzles').document(puzzle_id).get().to_dict['moves_to_solve']}")
